...
% im1: the 1st image
% im2: the 2nd image
% im1_pts: n x 2 double 图一的控制点
% im2_pts: n x 2 double 图二的控制点
...
% tri: m x 3 double, m is the number of triangles 三角剖分面片
...
for i = 1 : length(t)
    ...
    barycentric(i,:) = A\[x_temp(i);y_temp(i);1];
end
...
im1_morph = zeros(d);
for i=1:size(im1_morph,1)
    for j=1:size(im1_morph,2)
        if(isnan(x_morph(i,j))||isnan(y_morph(i,j)))
        else
            im1_morph(i,j,:)=im1(round(x_morph(i,j)),round(y_morph(i,j)),:);
        end
    end
end
...
for i=1:size(im2_morph,1)
    for j=1:size(im2_morph,2)
        if(isnan(x_morph(i,j))||isnan(y_morph(i,j)))
        else
            im2_morph(i,j,:)=im2(round(x_morph(i,j)),round(y_morph(i,j)),:);
        end
    end
end
